...
class Action {
  constructor (docker, image, props) {
    this.docker = docker
    this.image = image
    this.container = null
    this.http_port = 8080
  ...
  invoke (parameters) {
    ...
    return Credentials.getWskProps().then(props => {
      return this.http_post(this.http_url() + '/run', {authKey: this.props.AUTH_KEY, value: parameters})
    })
  }
  ...
  get_host_config () {
    ...
    return {Env: this.get_host_env(), ExposedPorts, PortBindings}
  }

  get_host_env () {
    return [
      `EDGE_HOST=${this.props.EDGE_HOST}:443`,
