...
class Action {
  ...
  start () {
    ...
    // Docker options to expose HTTP on container and bind to random host port.
    ...
    return new Promise((resolve, reject) => {
      this.docker.run(this.image, [], process.stdout, { ExposedPorts }, { PortBindings }, (err, data, container) => {
  ...
  http_url () {
    const httpPort = this.container.Ports
      .filter(port => port.PrivatePort === 8080).pop()
